	...
	if strings.HasPrefix(remoteURL, "git@") {
		...
		if userName := u.User.Username(); userName != "" {
			if _, passwordSet := u.User.Password(); !passwordSet {
				// For private git repositories, GitHub and Azure DevOps supports git url like http://pat@gitbhub.com/user/repo.git
				// However git-lfs requires the credential in "user:pat" format. So we need to add a dummy user name
				// Other git services like GitLab, BitBucket only supports "user:pat" credential.
