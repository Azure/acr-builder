...
func TestGetBuildDockerRunArgs(t *testing.T) {
	executor := &Executor{}
	actualCmds := executor.getDockerRunArgsForStep("volName", "stepWorkDir", &graph.Step{ID: "id", Build: "-f Dockerfile .", Envs: []string{"foo=bar", "HOME=qux"}}, "", "docker build -f Dockerfile .")
...
func TestGetNonBuildDockerRunArgs(t *testing.T) {
	executor := &Executor{}
	actualCmds := executor.getDockerRunArgsForStep("volName", "stepWorkDir", &graph.Step{ID: "id", Envs: []string{"foo=bar"}}, "", "hello-world")
